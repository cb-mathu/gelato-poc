import { h, cloneVNode, Slots, DefineComponent, SetupContext } from 'vue';
...
function _render({
  ...
  if (as === 'template') {
    if (Object.keys(passThroughProps).length > 0 || Object.keys(attrs).length > 0) {
      ...
      return cloneVNode(firstChild, passThroughProps as Record<string, unknown>);
    }
  ...
  return h(as as string | Object, passThroughProps, children);
}

export const simpleComponentFactory = (el: string, className: string) => {
  const simpleComponent = (props: Record<string, unknown>, context: SetupContext) => {
    return h(
      el,
      {
        class: className,
